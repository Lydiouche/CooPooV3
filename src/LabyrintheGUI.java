...
import java.awt.*;
    ...
    public class LabyrintheGUI extends JFrame {
        ...
        // Définition des limites basées sur ton fichier Plateau
        ...
        public LabyrintheGUI(Plateau plateau, Personne joueur) {
            ...
            setSize(500, 600);
            setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE); // On ferme via la console
            JPanel grillePanel = new JPanel(new GridLayout(nbLignes, nbColonnes));
            // CRÉATION INITIALE DES CASES
            for (int y = MAX_Y; y >= MIN_Y; y--) {
                for (int x = MIN_X; x <= MAX_X; x++) {
                    JPanel p = new JPanel(new BorderLayout());
                    p.setBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY));

                    // On stocke la case dans notre tableau 2D (avec décalage d'index)
                    casesPhysiques[x - MIN_X][y - MIN_Y] = p;
                    grillePanel.add(p);
                }
            }
            add(grillePanel);
            actualiser(); // Premier dessin
            setVisible(true);
        }

        public void actualiser() {
            // On parcourt le plateau (Y de haut en bas pour l'affichage)
            for (int y = MAX_Y - 1; y >= MIN_Y; y--) {
                for (int x = MIN_X; x <= MAX_X; x++) {
                    JPanel p = casesPhysiques[x - MIN_X][y - MIN_Y];
                    p.removeAll(); // On vide juste le contenu (le texte/label)
